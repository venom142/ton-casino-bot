...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; outline: none; }
        ...
        .nav { display: flex; gap: 4px; padding: 10px; background: rgba(0,0,0,0.5); }
        .tab { flex: 1; padding: 10px; background: #222; border: 1px solid #444; border-radius: 10px; font-size: 10px; text-align: center; font-weight: bold; opacity: 0.6; }
        .tab.active { opacity: 1; border-color: #f0f; background: rgba(255,0,255,0.2); }
        .page { height: calc(100vh - 60px); padding: 15px; display: flex; flex-direction: column; overflow-y: auto; }
        .card { background: rgba(0,0,0,0.85); border: 1px solid #0ff; padding: 15px; border-radius: 15px; text-align: center; margin-bottom: 15px; }
        .btn { width: 100%; padding: 16px; border-radius: 12px; border: none; background: linear-gradient(135deg, #f0f, #60f); color: #fff; font-weight: 900; font-size: 16px; box-shadow: 0 4px 10px rgba(0,0,0,0.3); }
        .btn:active { transform: scale(0.96); }
        .btn:disabled { opacity: 0.5; }
        input, select { width: 100%; padding: 12px; background: #111; border: 1px solid #333; color: #0ff; border-radius: 10px; margin: 8px 0; text-align: center; font-size: 16px; }
        .reels { display: flex; justify-content: center; gap: 8px; margin: 15px 0; }
        .window { width: 30%; height: 90px; background: #000; border: 2px solid #f0f; border-radius: 12px; overflow: hidden; position: relative; }
...
<body>
    ...
    <div id="p1" class="page">
        <div class="card">
            ...
            <div style="font-size:36px; font-weight:900; color:#0ff;" id="v-bal">0.00</div>
            <select id="bet-val">
    ...
    <div id="p2" class="page hidden">
        <div class="card">
            ...
            <div style="display:flex; justify:space-around; margin:15px 0;">
                <div><p id="v-s">0</p><span>Игр</span></div>
                <div><p id="v-w">0</p><span>Побед</span></div>
            </div>
    ...
    <div id="p3" class="page hidden">
        <div class="card">
            <h3>ДЕПОЗИТ</h3>
            <p style="font-size:12px; margin:10px 0;">Переведи TON на кошелек:</p>
            ...
            <p style="font-size:12px; margin-top:10px;">Комментарий к переводу:</p>
            <div id="v-cid" style="font-size:18px; color:#f0f; font-weight:bold; margin-top:5px;">ID_...</div>
        </div>
    </div>
    ...
    <script>
        ...
        async function spin() {
            ...
            [1,2,3].forEach((id, i) => {
                const s = document.getElementById('rs'+id);
                s.children[29].innerText = data.r[i]; // Ставим результат в конец
            ...
            setTimeout(() => {
                ...
                [1,2,3].forEach(id => {
                    ...
                    s.style.transition = 'none';
                    s.style.transform = 'translateY(0)';
                    s.children[0].innerText = data.r[id-1]; // Оставляем символ
                });
            }, 3000);
        }
