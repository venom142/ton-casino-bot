...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            /* ===== NEW UI ONLY: bottom navigation, no old top nav ===== */
            ...
            .bottom-nav {
                ...
                box-shadow: 0 0 24px rgba(0, 240, 255, 0.20), inset 0 0 18px rgba(255, 0, 255, 0.06);
                backdrop-filter: blur(10px);
            }
            .bottom-nav .tab {
            ...
            .bottom-nav .tab.active {
                color: #fff !important;
                background: linear-gradient(135deg, rgba(0,240,255,0.25), rgba(255,0,255,0.18)) !important;
                border-color: rgba(0,240,255,0.58) !important;
                box-shadow: 0 0 14px rgba(0,240,255,0.22) !important;
            }
            @media (max-width: 370px) {
                .bottom-nav { height: 62px; gap: 2px; padding: 6px; }
                .bottom-nav .tab { font-size: 16px !important; }
                .bottom-nav .tab span { font-size: 8px; }
            }
    ...
    <body>
        ...
        <div id="pg7" class="page">
            <div class="card" id="promoSection">
...
const uid = tg.initDataUnsafe?.user?.id || 123456789;
            ...
            function sh(n, targetId = null) {
                document.querySelectorAll('.page').forEach(e => e.classList.remove('active'));
                const page = document.getElementById('pg' + n);
                if (page) page.classList.add('active');

                document.querySelectorAll('.tab').forEach(e => e.classList.remove('active'));
                let activeTab = null;
                if (targetId) {
                    activeTab = document.querySelector('.tab[data-page="' + n + '"][data-target="' + targetId + '"]');
                }
                if (!activeTab) {
                    activeTab = document.querySelector('.tab[data-page="' + n + '"]');
                }
            ...
            function copy(t) { navigator.clipboard.writeText(t); gameAlert("✅ Скопировано в буфер"); }
            ...
            function withdraw() {
                const a = await gamePrompt('Введи кошелёк для вывода', 'Адрес кошелька'); if(!a) return;
                const sum = await gamePrompt('Сколько 💎 вывести?', 'Сумма вывода'); if(!sum) return;
