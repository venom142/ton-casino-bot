...
const Promo = mongoose.model('Promo', {
    ...
    amount: Number,
    limit: { type: Number, default: 1 }, // Максимальное кол-во активаций
    used: { type: Number, default: 0 }    // Сколько уже использовали
});
...
// === 🛠 АДМИН-КОМАНДЫ ===
// Формат: /addpromo [КОД] [СУММА] [ЛИМИТ]
bot.onText(/\/addpromo (.+) (.+) (.+)/, async (msg, match) => {
    ...
    try {
        ...
        bot.sendMessage(msg.chat.id, `✅ Промо **${code}** создан!\n💰 Сумма: **${amount} TON**\n👥 Лимит: **${limit} чел.**`);
    } catch (e) { bot.sendMessage(msg.chat.id, "❌ Ошибка: код уже существует."); }
});
...
app.post('/api/promo', async (req, res) => {
    ...
    if (!u || !pr) return res.json({ err: "НЕВЕРНЫЙ КОД" });
    if (u.promo.includes(pr.code)) return res.json({ err: "УЖЕ ИСПОЛЬЗОВАНО" });
    if (pr.used >= pr.limit) return res.json({ err: "ЛИМИТ ИСЧЕРПАН" });

    u.balance = Number((u.balance + pr.amount).toFixed(2));
    u.promo.push(pr.code);
    pr.used += 1;
    ...
    <style>
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; margin: 0; padding: 0; }
        ...
        .bg-grid { 
            ...
            background-image: 
                linear-gradient(rgba(110, 0, 255, 0.15) 1px, transparent 1px),
                linear-gradient(90deg, rgba(110, 0, 255, 0.15) 1px, transparent 1px);
        ...
        .reel { 
            width: 30%; height: 85px; background: rgba(0,0,0,0.5); 
            border: 2px solid #f0f; border-radius: 18px; 
            font-size: 40px; display: flex; align-items: center; justify-content: center;
        }
        .reel.spinning { animation: blur-spin 0.1s linear infinite; filter: blur(4px); }
        @keyframes blur-spin { 0% { transform: translateY(-5px); } 50% { transform: translateY(5px); } 100% { transform: translateY(-5px); } }
        .btn-spin { 
            width: 100%; padding: 22px; border-radius: 20px; border: none;
            background: linear-gradient(135deg, #ff00ff, #6e00ff); 
            color: #fff; font-size: 20px; font-weight: 900; text-transform: uppercase;
...
<body>
    ...
    <div class="main-container">
        ...
        <div id="p-stat" class="hidden">
            <div class="card" style="text-align: left;"><h3>СТАТЫ</h3><p>Игр: <span id="v-s">0</span></p><p>Побед: <span id="v-w">0</span></p></div>
        </div>
        <div id="p-set" class="hidden">
            <div class="card">
                <button class="set-btn" onclick="tglM()" id="m-btn">🔊 МУЗЫКА: ВЫКЛ</button>
                <button class="set-btn" style="background:#0ff; color:#000;" onclick="askPromo()">🎟 ПРОМОКОД</button>
            </div>
        </div>
    </div>
    <script>
        ...
        async function sync() {
            ...
            if(!document.getElementById('r1').classList.contains('spinning')) {
        ...
        async function spin() {
            ...
            try {
                ...
                setTimeout(() => {
                    reels.forEach((r, i) => { r.classList.remove('spinning'); r.innerText = d.r[i]; });
                    sync();
                    btn.disabled = false;
                    if(d.win > 0) { tg.HapticFeedback.notificationOccurred('success'); tg.showAlert("🔥 ПОБЕДА!"); }
                }, 800);
            } catch (e) { reels.forEach(r => r.classList.remove('spinning')); btn.disabled = false; }
        }
