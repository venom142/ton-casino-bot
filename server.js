    ...
    <body>
        ...
        <script>
            ...
            function buildStrip(targetSymbol) {
                let html = '';
                // 34 символа до финала = плавнее прокрутка без сильного лага
                for (let i = 0; i < 34; i++) {
                    html += '<div class="sym">' + syms[Math.floor(Math.random() * syms.length)] + '</div>';
                }
                html += '<div class="sym">' + targetSymbol + '</div>';
                // запас после финала, чтобы Telegram WebView не обрезал анимацию
                for (let i = 0; i < 3; i++) {
                    html += '<div class="sym">' + syms[Math.floor(Math.random() * syms.length)] + '</div>';
                }
                return html;
            ...
            function animateReel(el, targetY, delay, duration) {
                if (!el) return;
                setTimeout(function () {
                    el.classList.add('spinning');
                    el.style.transition = 'transform ' + duration + 'ms cubic-bezier(0.08, 0.78, 0.12, 1)';
            ...
            async function playSpin() {
                ...
                try {
                    ...
                    // force reflow чтобы анимация точно стартовала
                    void s1.offsetWidth;
                    void s2.offsetWidth;
                    void s3.offsetWidth;

                    const targetY = -(34 * 120);

                    animateReel(s1, targetY, 40, 2300);
                    animateReel(s2, targetY, 220, 2700);
                    animateReel(s3, targetY, 400, 3100);
