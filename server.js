...
// ==========================================
let crashRoundId = 1;
const crashAbuseStats = {}; // uid -> { nearCrashStreak }

function addCrashSuspicious(uid, reason, extra = '') {
    const id = uid ? uid.toString() : 'unknown';
    const msg = extra ? `${id}: ${reason} (${extra})` : `${id}: ${reason}`;
    if (!crashState.suspiciousEvents.includes(msg)) {
        crashState.suspiciousEvents.push(msg);
    }
}

function crashLog(uid, bet, cashoutMultiplier, crashPoint, winSum, status) {
    console.log(
        `ROUND_ID=${crashState.roundId} UID=${uid} BET=${bet} CASHOUT_MULTIPLIER=${cashoutMultiplier || '-'} CRASH_POINT=${crashPoint || '-'} WIN_SUM=${winSum || 0} STATUS=${status}`
    );
}
...
function sendCrashRoundReport() {
    try {
        ...
        const cashed = entries.filter(([_, b]) => b.cashedOut).length;
        const lost = total - cashed;

        for (const [uid, b] of entries) {
            if (b.cashedOut && b.cashoutMultiplier) {
                const diff = crashState.crashPoint - b.cashoutMultiplier;
                if (diff >= 0 && diff <= 0.05) {
                    addCrashSuspicious(uid, "cashout очень близко к взрыву", `${b.cashoutMultiplier.toFixed(2)}x / ${crashState.crashPoint.toFixed(2)}x`);
                    crashAbuseStats[uid] = crashAbuseStats[uid] || { nearCrashStreak: 0 };
                    crashAbuseStats[uid].nearCrashStreak++;
                    if (crashAbuseStats[uid].nearCrashStreak >= 3) {
                        addCrashSuspicious(uid, "слишком частые идеальные cashout подряд", `${crashAbuseStats[uid].nearCrashStreak} раз`);
                    }
                } else {
                    crashAbuseStats[uid] = crashAbuseStats[uid] || { nearCrashStreak: 0 };
                    crashAbuseStats[uid].nearCrashStreak = 0;
                }
                crashLog(uid, b.bet, b.cashoutMultiplier.toFixed(2), crashState.crashPoint.toFixed(2), b.winSum, "CASHED_OUT");
            } else {
                crashLog(uid, b.bet, "-", crashState.crashPoint.toFixed(2), 0, "LOST");
            }
        }

        const suspicious = crashState.suspiciousEvents.length
            ? crashState.suspiciousEvents.map(x => "⚠️ " + x).join("\n")
            : "✅ Нет";

        const report =
`🚀 CRASH ROUND REPORT
Раунд: #${crashState.roundId}
Взорвалась на: ${crashState.crashPoint.toFixed(2)}x
...
${suspicious}`;
        ...
        bot.sendMessage(CONFIG.ADMIN_ID, report).catch(() => {});
    } catch (e) {
...
const crashState = {
    ...
    suspiciousEvents: [],
    cashoutAttempts: {} // uid -> timestamps
};
    ...
    if (crashState.status === 'betting') {
        ...
        if (currentMult >= crashState.crashPoint) {
            ...
            setTimeout(() => {
                ...
                crashState.bets = {};
                crashState.suspiciousEvents = [];
                crashState.cashoutAttempts = {};
...
app.post('/api/crash/bet', async (req, res) => {
    try {
        ...
        if (crashState.bets[uid]) {
            addCrashSuspicious(uid, "повторная ставка в одном раунде");
            crashLog(uid, crashState.bets[uid].bet, "-", "-", 0, "DUPLICATE_BET");
...
app.post('/api/crash/cashout', async (req, res) => {
    try {
        ...
        if (currentMult >= crashState.crashPoint) {
            crashLog(uid, myBet.bet, currentMult.toFixed(2), crashState.crashPoint.toFixed(2), 0, "LATE_CASHOUT");
        ...
        if (user) {
            ...
            user.wins++;
            await user.save();
            crashLog(uid, myBet.bet, currentMult.toFixed(2), crashState.crashPoint.toFixed(2), winSum, "CASHOUT");
...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .reel-cont { display: flex; justify-content: center; gap: 12px; margin: 28px 0; perspective: 700px; }
            .reel {
                width: 90px; height: 120px; background: linear-gradient(180deg,#02030a,#080b18,#02030a);
                border: 2px solid var(--neon-cyan); border-radius: 16px; overflow: hidden; position: relative;
                box-shadow: inset 0 0 18px rgba(0,240,255,0.18), 0 0 18px rgba(0,240,255,0.25);
                transform: translateZ(0);
            }
            .reel:before, .reel:after {
                content: ""; position: absolute; left: 0; right: 0; height: 32px; z-index: 2; pointer-events: none;
            }
            .reel:before { top: 0; background: linear-gradient(180deg, rgba(0,0,0,.9), transparent); }
            .reel:after { bottom: 0; background: linear-gradient(0deg, rgba(0,0,0,.9), transparent); }
            .strip {
            ...
            .strip.spinning .sym { filter: blur(.45px) drop-shadow(0 0 8px rgba(255,255,255,0.16)); }
            .sym {
                height: 120px; display: flex; align-items: center; justify-content: center;
                font-size: 58px; line-height: 120px;
            ...
            .sym { filter: none !important; }
            .back-video { opacity: .25 !important; }
    ...
    <body>
        ...
        <script>
            ...
            // --- ИГРА: СЛОТЫ — NO LAG MODE ---
            function setReelFinal(id, symbol) {
            ...
            function quickReelTick(ids) {
                ids.forEach(function(id) {
                    const el = document.getElementById(id);
                    if (!el) return;
                    el.innerHTML = '<div class="sym">' + syms[Math.floor(Math.random() * syms.length)] + '</div>';
                });
            }
                ...
                try {
                    ...
                    const spinTimer = setInterval(function () {
                        ...
                        if (ticks >= 6) {
                            ...
                            setReelFinal('s1', d.result[0]);
                            setTimeout(function(){ setReelFinal('s2', d.result[1]); }, 120);
                            setTimeout(function(){ setReelFinal('s3', d.result[2]); }, 240);
