...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .crash-monitor { position: relative; overflow: hidden; }
            .rocket-visual {
                position: absolute;
                left: 50%;
                bottom: 18px;
                transform: translateX(-50%) translateY(70px) rotate(-12deg);
                font-size: 42px;
                opacity: 0;
                z-index: 2;
            ...
            .rocket-visual.fly { opacity: 1; animation: rocketFloat 1.1s ease-in-out infinite alternate; }
            .rocket-visual.cashout { opacity: 1; filter: drop-shadow(0 0 22px rgba(0,255,120,0.95)); animation: cashoutFlash .45s ease; }
            .rocket-visual.boom { opacity: 1; filter: drop-shadow(0 0 24px rgba(255,40,90,0.95)); animation: boomPop .55s ease; }
            .rocket-trail {
                position: absolute;
                left: 50%;
                bottom: 12px;
                ...
                border-radius: 999px;
                background: linear-gradient(to top, rgba(255,0,255,0), rgba(0,240,255,0.7), rgba(255,255,255,0.9));
                opacity: 0;
                z-index: 1;
                filter: blur(1px);
                pointer-events: none;
            }
            .rocket-trail.show { opacity: .8; animation: trailPulse .55s ease-in-out infinite alternate; }
            .crash-monitor.cashout-glow { box-shadow: 0 0 28px rgba(0,255,120,0.55), inset 0 0 30px rgba(0,255,120,0.12); }
            .crash-monitor.boom-glow { box-shadow: 0 0 34px rgba(255,0,90,0.65), inset 0 0 34px rgba(255,0,90,0.16); }
            @keyframes rocketFloat {
                from { margin-left: -8px; filter: drop-shadow(0 0 14px rgba(0,240,255,0.75)); }
                to { margin-left: 8px; filter: drop-shadow(0 0 24px rgba(255,0,255,0.75)); }
            }
            @keyframes trailPulse { from { height: 70px; opacity: .45; } to { height: 112px; opacity: .9; } }
            @keyframes boomPop {
                0% { transform: translateX(-50%) scale(1) rotate(-12deg); }
                45% { transform: translateX(-50%) scale(1.45) rotate(8deg); }
            ...
            @keyframes cashoutFlash {
                ...
                100% { transform: translateX(-50%) scale(1) rotate(-12deg); }
            }
...
const uid = tg.initDataUnsafe?.user?.id || 123456789;
            ...
            function setRocketState(state, multiplier = 1) {
                ...
                if (state === 'idle') {
                    rocket.style.transform = 'translateX(-50%) translateY(70px) rotate(-12deg)';
                    return;
                }
                ...
                if (state === 'fly') {
                    ...
                    const y = 70 - (m - 1) * 5.2;
                    rocket.style.transform = 'translateX(-50%) translateY(' + Math.max(-86, y) + 'px) rotate(-12deg)';
                ...
                if (state === 'cashout') {
                    ...
                    setTimeout(() => { rocket.innerText = '🚀'; monitor.classList.remove('cashout-glow'); }, 650);
                    return;
                }
                ...
                if (state === 'boom') {
                    ...
                    setTimeout(() => { rocket.innerText = '🚀'; monitor.classList.remove('boom-glow'); }, 800);
                }
            }
                ...
                try {
                    ...
                    if (d.status === 'betting') {
                        ...
                        if (d.bet > 0 && !d.cashedOut) {
                            ...
                            setCrashResult("✅ Забрал куш: +" + formatBal(d.winSum) + " 💎", "win");
                            btn.disabled = true;
            ...
            async function cashoutCrashGlobal() {
                ...
                try {
                    ...
                    if(d.err) {
                        ...
                        setRocketState('cashout');
                        gameAlert("✅ Забрал +" + formatBal(d.winSum) + " 💎 на " + d.multiplier + "x");
