...
const uid = tg.initDataUnsafe?.user?.id || 123456789;
            ...
            function setRocketState(state, multiplier = 1, cashedOut = false) {
                ...
                if (state === 'fly') {
                    ...
                    const y = 38 - (m - 1) * 3.2;
                    rocket.style.transform = 'translate(-50%, -50%) translateY(' + Math.max(-54, y) + 'px) scale(0.9) rotate(-12deg)';
                ...
                if (state === 'cashout') {
                    ...
                    setTimeout(() => {
                        rocket.innerText = '🚀';
                        monitor.classList.remove('cashout-glow');
                    }, 650);
                    return;
                }
                ...
                if (state === 'boom') {
                    ...
                    setTimeout(() => {
                        rocket.innerText = '🚀';
                        monitor.classList.remove('boom-glow');
                    }, 800);
                }
            }
            ...
            async function pollCrashState() {
                try {
                    ...
                    if (d.status === 'betting') {
                        ...
                        setRocketState('boom', 1, d.bet > 0 && d.cashedOut);
                        stopCrashAnimation();
