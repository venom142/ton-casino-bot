...
// ==========================================
let crashRoundId = 1;
const crashAbuseStats = {}; // uid -> { nearCrashStreak, cashoutsInSecond, lastCashoutTime }
const currentSuspicious = []; // Список подозрительных действий в текущем раунде

function addSuspiciousCrash(uid, reason) {
    currentSuspicious.push({ uid, reason });
    console.log(`⚠️ SUSPICIOUS [UID=${uid}]: ${reason}`);
}

function sendCrashRoundReport() {
    try {
        const entries = Object.entries(crashState.bets || {});
        const total = entries.length;
        if (total === 0) { currentSuspicious.length = 0; return; }
        
        let cashed = 0;
        let lost = 0;

        for (const [uid, myBet] of entries) {
            if (myBet.cashedOut) {
                cashed++;
                
                // Проверка на cashout очень близко к взрыву
                if (crashState.crashPoint - myBet.cashoutMultiplier <= 0.05) {
                    addSuspiciousCrash(uid, "cashout очень близко к взрыву");
                    
                    if (!crashAbuseStats[uid]) crashAbuseStats[uid] = { nearCrashStreak: 0, cashoutsInSecond: 0, lastCashoutTime: 0 };
                    crashAbuseStats[uid].nearCrashStreak++;
                    
                    if (crashAbuseStats[uid].nearCrashStreak >= 3) {
                         addSuspiciousCrash(uid, "слишком частые идеальные cashout подряд");
                    }
                } else {
                     if (crashAbuseStats[uid]) crashAbuseStats[uid].nearCrashStreak = 0;
                }
            } else {
                lost++;
                if (crashAbuseStats[uid]) crashAbuseStats[uid].nearCrashStreak = 0;
        ...
        let suspiciousText = "✅ Нет";
        if (currentSuspicious.length > 0) {
            suspiciousText = currentSuspicious.map(s => `- UID ${s.uid}: ${s.reason}`).join('\n');
        }

        const report =
`🚀 CRASH REPORT
Раунд: #${crashState.roundId}
Взорвалась на: ${crashState.crashedMultiplier.toFixed(2)}x
...
${suspiciousText}`;
        ...
        bot.sendMessage(CONFIG.ADMIN_ID, report).catch(e => console.log("Crash report send error:", e.message));
        
        // Очищаем список подозрительных для следующего раунда
        currentSuspicious.length = 0;
        
    } catch (e) {
        console.log("Crash report error:", e.message);
    }
}
...
const crashState = {
    ...
    bets: {} // uid -> { bet, cashedOut, winSum, cashoutMultiplier }
};
...
setInterval(() => {
    ...
    if (crashState.status === 'betting') {
        if (now >= crashState.bettingEndsAt) {
            ...
            console.log(`ROUND_ID=${crashState.roundId} STATUS=STARTED CRASH_POINT_HIDDEN`);
        }
        ...
        if (currentMult >= crashState.crashPoint) {
            ...
            console.log(`ROUND_ID=${crashState.roundId} STATUS=CRASHED CRASH_POINT=${crashState.crashPoint}`);
            sendCrashRoundReport();
            
            // Ожидаем 3 секунды и начинаем новый раунд
            setTimeout(() => {
                crashRoundId++;
                crashState.roundId = crashRoundId;
...
app.post('/api/crash/bet', async (req, res) => {
    try {
        ...
        if (!uid || isNaN(safeBet) || safeBet < SETTINGS.minBet) {
            return res.json({ err: "Ошибка ставки" });
        }

        if (crashState.status !== 'betting') return res.json({ err: "Ставки уже закрыты!" });
        if (crashState.bets[uid]) {
            addSuspiciousCrash(uid, "повторная ставка в одном раунде");
        ...
        crashState.bets[uid] = { bet: safeBet, cashedOut: false, winSum: 0, cashoutMultiplier: 0 };
        console.log(`ROUND_ID=${crashState.roundId} UID=${uid} BET=${safeBet} CASHOUT_MULTIPLIER=N/A CRASH_POINT=N/A WIN_SUM=0 STATUS=BET_PLACED`);
...
app.post('/api/crash/cashout', async (req, res) => {
    try {
        ...
        if (now - crashAbuseStats[uid].lastCashoutTime <= 1000) {
            crashAbuseStats[uid].cashoutsInSecond++;
        } else {
            crashAbuseStats[uid].cashoutsInSecond = 1;
            crashAbuseStats[uid].lastCashoutTime = now;
        }
        
        if (crashAbuseStats[uid].cashoutsInSecond > 4) {
            addSuspiciousCrash(uid, "слишком много cashout-запросов");
        }
        ...
        if (!myBet) {
            addSuspiciousCrash(uid, "cashout без активной ставки");
        ...
        myBet.winSum = winSum;
        myBet.cashoutMultiplier = currentMult;
        ...
        if (user) {
            ...
            user.wins++;
            await user.save();
            console.log(`ROUND_ID=${crashState.roundId} UID=${uid} BET=${myBet.bet} CASHOUT_MULTIPLIER=${currentMult.toFixed(2)} CRASH_POINT=HIDDEN WIN_SUM=${winSum} STATUS=CASHED_OUT`);
...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .crash-x { font-size: 54px; font-weight: 900; color: #fff; text-shadow: 0 0 20px #fff; transition: color 0.2s ease, text-shadow 0.2s ease, transform 0.15s ease; font-variant-numeric: tabular-nums; font-feature-settings: "tnum"; min-width: 180px; text-align: center; display: inline-block; letter-spacing: 1px; }
            ...
            .strip.spinning .sym { filter: none !important; }
            .sym { filter: drop-shadow(0 0 4px rgba(255,255,255,0.12)) !important; }
            .back-video { opacity: .28 !important; }
            .card, .stat-card, .bal-panel { backdrop-filter: none !important; -webkit-backdrop-filter: none !important; }
    ...
    <body>
        ...
        <div id="pg5" class="page">
            <div class="card">
                ...
                <p style="color:#666; font-size:11px; margin-top:20px;">Версия клиента: 3.2 (UI Fixes Edition)</p>
            </div>
        </div>
        ...
        <script>
            ...
            function setCrashMultiplier(value) {
                const txt = Number(value).toFixed(2) + "x";
            ...
            function sh(n) {
                ...
                if(n === 2) {
                    if(!crashPollInterval) crashPollInterval = setInterval(pollCrashState, 700);
                    pollCrashState();
                } else {
            ...
            // --- ИГРА: СЛОТЫ ---
            function setReelSymbol(id, symbol, glow) {
                const el = document.getElementById(id);
                if (!el) return;
                el.style.transition = 'transform 120ms ease, opacity 120ms ease';
                el.style.transform = glow ? 'scale(1.08)' : 'scale(1)';
                el.style.opacity = glow ? '1' : '.82';
                el.innerHTML = '<div class="sym">' + symbol + '</div>';
            }

            async function playSpin() {
                if (isSlotGame) return;
                const bet = parseFloat(document.getElementById('bet1').value);
                if (isNaN(bet) || bet <= 0) return tg.showAlert("Введите ставку");
                ...
                try {
                    ...
                    const timer = setInterval(function(){
                        ids.forEach(function(id){
                            setReelSymbol(id, syms[Math.floor(Math.random()*syms.length)], tick % 2 === 0);
                        });
                        tick++;
                        if (tick >= 10) {
                            clearInterval(timer);
                            setTimeout(function(){ setReelSymbol('s1', d.result[0], true); }, 80);
                            setTimeout(function(){ setReelSymbol('s2', d.result[1], true); }, 220);
                            setTimeout(function(){ setReelSymbol('s3', d.result[2], true); }, 360);

                            setTimeout(function(){
                                updateBal(d.balance);
                                if (d.winSum > 0) {
                                    tg.showAlert("🎉 ВЫИГРЫШ: " + formatBal(d.winSum) + " 💎");
            ...
            async function pollCrashState() {
                try {
                    ...
                    if (d.status === 'betting') {
                        ...
                        cx.style.color = "#00ff00";
                        setCrashMultiplier(d.currentMultiplier);
                        ...
                        setCrashMultiplier(d.crashedMultiplier);
