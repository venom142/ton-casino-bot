    ...
    <head>
        ...
        <style>
            ...
            .vip-title {
                ...
                background: linear-gradient(90deg, #b6ff00, #00f0ff, #ff00ff);
            ...
            .modal-overlay .modal-box {
                ...
                max-width: 340px;
                text-align: center;
                box-shadow: 0 0 30px rgba(0,240,255,0.3), inset 0 0 20px rgba(255,0,255,0.1);
                transform: scale(0.9);
...
const tg = window.Telegram.WebApp;
            ...
            function openModal(options) {
                return new Promise((resolve) => {
                    ...
                    const title = document.getElementById('modalCustomTitle');
                    const desc = document.getElementById('modalCustomDesc');
                    const inp = document.getElementById('modalCustomInput');
                    const btnConfirm = document.getElementById('modalBtnConfirm');
                    const btnCancel = document.getElementById('modalBtnCancel');

                    title.innerText = options.title || 'Внимание';
                    ...
                    if (options.hasInput) {
                        ...
                        inp.value = '';
                        inp.type = options.inputType || 'text';
                        setTimeout(() => inp.focus(), 100);
                    } else {
                        inp.style.display = 'none';
                    }

                    btnConfirm.innerText = options.btnConfirmText || '✅ Понятно';
                    
                    if (options.showCancel) {
                        btnCancel.style.display = 'block';
                        btnCancel.innerText = options.btnCancelText || '❌ Отмена';
                    } else {
                    ...
                    btnConfirm.onclick = () => {
                        ...
                        resolve(options.hasInput ? inp.value.trim() : true);
                    };
            ...
            function gameAlert(msg) {
                ...
                if (t.includes('Скопировано')) {
                    showToast(t, 'success');
                    return;
                }

                let isError = t.includes('❌') || low.includes('ошибк') || low.includes('мало') || low.includes('недостат') || low.includes('невер') || low.includes('уже');
                let isSuccess = t.includes('✅') || t.includes('🎉') || t.includes('🎁') || low.includes('успешн') || low.includes('выигрыш') || low.includes('забрал');
...
const uid = tg.initDataUnsafe?.user?.id || 123456789;
            ...
            async function withdraw() {
                const a = await openModal({
                    title: '💎 VIP ХОТ ТАП',
                    desc: 'Введи кошелёк для вывода 💎',
                    hasInput: true,
                    placeholder: 'Адрес кошелька',
                    showCancel: true,
                    btnConfirmText: '✅ Подтвердить',
                    btnCancelText: '❌ Отмена'
                });
                if (!a) return;
                
                const sum = await openModal({
                    title: '🏦 Вывод средств',
                    desc: 'Укажи сумму ХОТ ТАП 💎',
                    hasInput: true,
                    inputType: 'number',
                    placeholder: 'Сумма',
                    showCancel: true,
                    btnConfirmText: '✅ Вывести',
                    btnCancelText: '❌ Отмена'
                });
                if (!sum) return;
            ...
            function flashSlotsWin() {
                ...
                if (!box) return;
                box.classList.remove('slots-win');
                void box.offsetWidth;
                box.classList.add('slots-win');
                setTimeout(() => box.classList.remove('slots-win'), 1900);
            }
        ...
        <div id="customModalOverlay" class="modal-overlay">
            <div class="modal-box">
                <div class="modal-title" id="modalCustomTitle">Внимание</div>
                <div class="modal-desc" id="modalCustomDesc">Текст</div>
                <input type="text" id="modalCustomInput" class="modal-input" style="display: none;">
                <div class="modal-buttons">
                    <button id="modalBtnCancel" class="btn-main dark" style="display: none;">❌ Отмена</button>
                    <button id="modalBtnConfirm" class="btn-main magenta">✅ Понятно</button>
                </div>
            </div>
        </div>
