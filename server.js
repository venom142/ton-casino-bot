...
const APP_URL = "https://your-app-name.onrender.com"; // ЗАМЕНИ НА СВОЮ ССЫЛКУ ОТ RENDER
...
// ИНИЦИАЛИЗАЦИЯ БОТА И КОМАНДЫ START
if (BOT_TOKEN) {
    ...
    console.log("BOT: СЛУШАЕТ КОМАНДУ /START");
}
...
app.post('/api/sync', async (req, res) => {
    try {
        ...
        res.json(u);
    } catch(e) { res.status(500).json({err: "Sync error"}); }
});
...
app.post('/api/spin', async (req, res) => {
    ...
    } catch(e) { res.json({err: "Spin error"}); }
});
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; outline: none; }
        body { height: 100vh; background: #000 url('https://files.catbox.moe/622ngf.jpg') center/cover; color: #fff; font-family: sans-serif; overflow: hidden; }
        .nav { display: flex; padding: 10px; gap: 5px; background: rgba(0,0,0,0.8); }
        .tab { flex: 1; padding: 12px; background: #111; border: 1px solid #333; border-radius: 10px; text-align: center; font-size: 10px; opacity: 0.5; font-weight: bold; }
...
<body>
    ...
    <div id="p3" class="page hidden">
        <div class="card">
            ...
            <div id="v-cid" class="copy-box" style="color:#f0f; border-color:#f0f; font-size:20px;" onclick="cp(this.innerText.split('_')[1])">ID_...</div>
        </div>
    </div>
    ...
    <script>
        ...
        function sw(n) { [1,2,3,4].forEach(i => { 
            const p = document.getElementById('p'+i); if(p) p.classList.toggle('hidden', i!==n);
            const t = document.getElementById('t'+i); if(t) t.classList.toggle('active', i===n);
        }); }
        ...
        async function spin() {
            ...
            try {
                ...
                setTimeout(() => { sync(); btn.disabled = false; if(d.win>0) tg.showAlert("ВЫИГРЫШ!"); }, 3000);
