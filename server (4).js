...
bot.on('callback_query', async (q) => {
    ...
    if (q.data.startsWith('withdraw_ok_')) {
        ...
        if (!user) return bot.sendMessage(q.message.chat.id, "❌ Ошибка профиля. Игрок не найден.");
        if (user.balance >= amount) {
            ...
            bot.sendMessage(q.message.chat.id, "❌ Ошибка! На момент подтверждения у игрока уже недостаточно средств.");
        }
        return;
    }
...
app.post('/api/crash', async (req, res) => {
    try {
        const { uid, bet, target } = req.body; const user = await User.findOne({ uid: uid.toString() });
        if (!user || user.balance < bet || bet < SETTINGS.minBet || target < 1.01) return res.json({ err: "Неверная ставка или икс!" });
        user.balance -= bet;
        let crashPoint = 1.00;
        if (Math.random() > 0.05) crashPoint = parseFloat((1 / Math.random() * 0.95).toFixed(2));
        if (crashPoint < 1.00) crashPoint = 1.00;
        let isWin = target <= crashPoint;
        let winSum = isWin ? Math.floor(bet * target) : 0;
        if (isWin) { user.balance += winSum; user.wins++; }
        user.spins++; await user.save();
        res.json({ crashPoint, isWin, winSum, balance: Math.floor(user.balance) });
    } catch (e) { res.json({ err: "Ошибка краша" }); }
});
...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .bal-wrapper { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; }
            .uid-badge { background: rgba(255,0,255,0.2); border: 1px solid var(--neon-magenta); padding: 5px 10px; border-radius: 8px; font-size: 12px; font-weight: bold; color: #fff; }
    ...
    <body>
        ...
        <div class="nav"><div class="tab active" onclick="sh(1)">🎰 Слоты</div><div class="tab" onclick="sh(2)">🚀 Краш</div><div class="tab" onclick="sh(3)">🏆 Топ</div><div class="tab" onclick="sh(4)">💎 Банк</div><div class="tab" onclick="sh(5)">⚙️ Настр.</div></div>
        <div id="pg1" class="page active"><div class="bal-wrapper"><div class="uid-badge">ID: <span id="uid1">...</span></div><div class="bal-box"><div style="font-size:10px; color:#aaa;">БАЛАНС ХОТ ТАП</div><div class="bal-val" id="bal1">0 💎</div></div></div><div class="reel-cont"><div class="reel"><div class="strip" id="s1"></div></div><div class="reel"><div class="strip" id="s2"></div></div><div class="reel"><div class="strip" id="s3"></div></div></div><div class="input-group"><div class="input-box" style="flex: 0.3;" onclick="chBet(-10, 'bet1')"><div style="text-align:center; font-size:24px; color:#aaa; margin-top:5px;">-</div></div><div class="input-box"><span style="text-align:center;">Ставка (💎)</span><input type="number" id="bet1" value="10" step="10" style="text-align:center;" readonly></div><div class="input-box" style="flex: 0.3;" onclick="chBet(10, 'bet1')"><div style="text-align:center; font-size:24px; color:#aaa; margin-top:5px;">+</div></div></div><button class="btn-main" onclick="playSpin()" id="btnSpin">КРУТИТЬ</button></div>
        <div id="pg2" class="page"><div class="bal-wrapper"><div class="uid-badge">ID: <span id="uid2">...</span></div><div class="bal-box"><div style="font-size:10px; color:#aaa;">БАЛАНС ХОТ ТАП</div><div class="bal-val" id="bal2">0 💎</div></div></div><div class="crash-monitor"><div class="crash-x" id="cX">1.00x</div><div class="crash-status" id="cMsg">Готов к запуску</div></div><div class="input-group"><div class="input-box"><span>Ставка (💎)</span><input type="number" id="bet2" value="10" step="10"></div><div class="input-box"><span>Авто-вывод (Икс)</span><input type="number" id="target2" value="2.0" step="0.1"></div></div><button class="btn-main magenta" onclick="playCrash()" id="btnCrash">ЗАПУСК РАКЕТЫ 🚀</button></div>
        <div id="pg3" class="page"><div class="card" style="padding:10px;"><h2 style="color:var(--neon-cyan); margin:10px 0; font-size:18px;">🏆 ЛУЧШИЕ ИГРОКИ</h2><div id="topList">Загрузка...</div></div></div>
        <div id="pg4" class="page"><div class="card"><h2 style="color:var(--neon-magenta); margin-top:0;">КАССА</h2><p style="color:#aaa; font-size:13px; text-align:left;">Пополнение автоматическое. Скопируй адрес ниже и отправь на него TON. <b>Обязательно укажи свой UID в комментарии (Memo)!</b> TON будут конвертированы в 💎 ХОТ ТАП.</p><div class="copy-box" onclick="copy('${CONFIG.WALLET}')">${CONFIG.WALLET}</div><p style="color:#ff0055; font-size:12px; font-weight:bold;">⚠️ ТВОЙ КОД ДЛЯ MEMO / COMMENT:</p><div class="copy-box" style="border-color:#ff0055; font-size:24px; font-weight:bold; color:#fff;" onclick="copy(uid.toString())" id="memoText">...</div><button class="btn-main" style="margin-top:20px; font-size:16px;" onclick="withdraw()">💸 ВЫВЕСТИ СРЕДСТВА</button><button class="btn-main dark" style="margin-top:10px; font-size:16px;" onclick="promo()">🎁 ВВЕСТИ ПРОМОКОД</button></div></div>
        <div id="pg5" class="page"><div class="card"><h2 style="color:var(--neon-cyan); margin-top:0;">НАСТРОЙКИ</h2><div class="bal-wrapper" style="justify-content:center; margin-bottom:30px;"><div class="uid-badge" style="font-size:18px; padding:10px 20px;">ТВОЙ ID: <span id="uid3" style="color:var(--gold);">...</span></div></div><button class="btn-main dark" style="margin-top:10px; font-size:16px; color:#fff; border-color:var(--neon-cyan);" onclick="toggleAudio()" id="audioBtn">🔊 ВЫКЛЮЧИТЬ ЗВУК</button><p style="color:#666; font-size:11px; margin-top:20px;">Версия клиента: 2.2 (Secure Payout Edition)</p></div></div>
        <script>
            const tg = window.Telegram.WebApp; tg.expand(); const uid = tg.initDataUnsafe?.user?.id || 123456789; let bal = 0, isGame = false;
            document.getElementById('uid1').innerText = uid; document.getElementById('uid2').innerText = uid; document.getElementById('uid3').innerText = uid; document.getElementById('memoText').innerText = uid;
            const syms = ['🍒','🔔','💎','7️⃣','🍋']; function initR() { let h = ''; for(let i=0; i<30; i++) h += '<div class="sym">'+syms[Math.floor(Math.random()*5)]+'</div>'; ['s1','s2','s3'].forEach(id => document.getElementById(id).innerHTML = h); } initR();
            function sh(n) { document.querySelectorAll('.page, .tab').forEach(e => e.classList.remove('active')); document.getElementById('pg'+n).classList.add('active'); document.querySelectorAll('.tab')[n-1].classList.add('active'); if(n===3) loadTop(); }
            function chBet(d, id) { let v = parseFloat(document.getElementById(id).value) + d; if(v < 10) v = 10; document.getElementById(id).value = Math.floor(v); }
            function copy(t) { navigator.clipboard.writeText(t); tg.showAlert("Скопировано!"); }
            function toggleAudio() { const a = document.getElementById('bgm'); a.muted = !a.muted; if(a.muted) { document.getElementById('audioBtn').innerText="🔈 ВКЛЮЧИТЬ ЗВУК"; } else { a.play().catch(e=>{}); document.getElementById('audioBtn').innerText="🔊 ВЫКЛЮЧИТЬ ЗВУК"; } }
            function updateBal(n) { bal = Math.floor(n); document.getElementById('bal1').innerText = bal + " 💎"; document.getElementById('bal2').innerText = bal + " 💎"; }
            async function upd() { try { const r = await fetch('/api/sync', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({uid})}); const d = await r.json(); updateBal(d.balance); } catch(e){} }
            async function loadTop() { document.getElementById('topList').innerHTML = "Загрузка..."; const r = await fetch('/api/leaderboard', {method:'POST'}); const d = await r.json(); let h = ''; d.forEach((u,i) => { let rank = i===0?'🥇':i===1?'🥈':i===2?'🥉':(i+1); h += '<div class="top-row"><div class="top-rank">'+rank+'</div><div style="flex:1; text-align:left; color:#ccc;">ID '+u.uid+'</div><div style="color:var(--neon-cyan); font-weight:900;">'+Math.floor(u.balance)+' 💎</div></div>'; }); document.getElementById('topList').innerHTML = h; }
            function draw(id, res) { const st = document.getElementById(id); st.style.transition = 'none'; st.style.transform = 'translateY(0)'; setTimeout(() => { st.innerHTML = '<div class="sym">'+res+'</div>' + st.innerHTML; st.style.transition = 'transform 2s cubic-bezier(0.15, 1, 0.3, 1)'; st.style.transform = 'translateY(-'+(st.children.length-1)*120+'px)'; }, 50); }
            async function playSpin() { if(isGame) return; const bet = parseFloat(document.getElementById('bet1').value); if(bet > bal) return tg.showAlert("Мало 💎 ХОТ ТАП!"); const a = document.getElementById('bgm'); if(a.paused && !a.muted) a.play().catch(e=>{}); isGame = true; const btn = document.getElementById('btnSpin'); btn.disabled = true; try { const r = await fetch('/api/spin', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({uid, bet})}); const d = await r.json(); if(d.err) { tg.showAlert(d.err); isGame=false; btn.disabled=false; return; } updateBal(bal - bet); initR(); draw('s1', d.result[0]); setTimeout(()=>draw('s2', d.result[1]), 300); setTimeout(()=>draw('s3', d.result[2]), 600); setTimeout(() => { updateBal(d.balance); if(d.winSum > 0) { tg.showAlert("🎉 ВЫИГРЫШ: " + Math.floor(d.winSum) + " 💎"); } isGame = false; btn.disabled=false; }, 2600); } catch(e) { isGame = false; btn.disabled=false; } }
            async function playCrash() { if(isGame) return; const bet = parseFloat(document.getElementById('bet2').value); const target = parseFloat(document.getElementById('target2').value); if(bet > bal) return tg.showAlert("Мало 💎 ХОТ ТАП!"); if(target < 1.01) return tg.showAlert("Икс должен быть больше 1.01"); const a = document.getElementById('bgm'); if(a.paused && !a.muted) a.play().catch(e=>{}); isGame = true; const btn = document.getElementById('btnCrash'); btn.disabled = true; const cx = document.getElementById('cX'); const cm = document.getElementById('cMsg'); cx.style.color = "#fff"; cx.innerText = "1.00x"; cm.innerText = "РАКЕТА ЛЕТИТ..."; try { const r = await fetch('/api/crash', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({uid, bet, target})}); const d = await r.json(); if(d.err) { tg.showAlert(d.err); isGame=false; btn.disabled=false; return; } updateBal(bal - bet); let curr = 1.00; const anim = setInterval(() => { curr += 0.01 + (curr * 0.015); if (curr >= Math.min(d.crashPoint, target)) { clearInterval(anim); if (d.isWin) { cx.innerText = target.toFixed(2) + "x"; cx.style.color = "#00ff00"; cm.innerText = "✅ ВЫВЕЛ +" + Math.floor(d.winSum) + " 💎"; } else { cx.innerText = d.crashPoint.toFixed(2) + "x"; cx.style.color = "#ff0000"; cm.innerText = "💥 ВЗРЫВ!"; } updateBal(d.balance); isGame = false; btn.disabled=false; } else { cx.innerText = curr.toFixed(2) + "x"; } }, 40); } catch(e) { isGame = false; btn.disabled=false; } }
            function withdraw() { const a = prompt("Кошелёк для вывода:"); if(!a) return; const sum = prompt("Сумма вывода (в 💎):"); if(!sum) return; fetch('/api/withdraw', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({uid, address:a, amount:parseFloat(sum)})}).then(r=>r.json()).then(d=>{ tg.showAlert(d.msg||d.err); upd(); }); }
            function promo() { const code = prompt("Введите промокод:"); if(!code) return; fetch('/api/promo', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({uid, promo:code})}).then(r=>r.json()).then(d=>{ tg.showAlert(d.msg||d.err); upd(); }); }
            setInterval(upd, 5000); upd(); document.getElementById('bgm').muted = false;
        </script>
    </body>
    </html>`);
});
